  ...
  return (
    <>
      ...
      <div className={`fixed inset-0 z-40 md:hidden transition-all duration-300 ${
        ...
        <div className={`absolute right-0 top-0 h-full w-80 bg-gradient-to-b from-slate-900 to-slate-800 shadow-xl transform transition-transform duration-300 ${
          ...
          <div className="flex flex-col h-full pt-20 px-6">
            {navItems.map((item, index) => (
              <button
                key={item.name}
                onClick={() => scrollToSection(item.href)}
                className={`text-left py-4 text-white/80 hover:text-white font-light transition-all duration-200 border-b border-white/10 animate-fade-in-up`}
